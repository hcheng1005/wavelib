...
*/
/*
This code is a C translation ( with some modifications) of Wavelet Software
...
static void wave_function(int nk, double dt, int mother, double param,
                          double scale1, double *kwave, double pi,
                          double *period1, double *coi1, fft_data *daughter) {
    ...
    *coi1 = fourier_factor * sqrt(2.0);
    ...
    if (re == 1) {
      for (k = 1; k <= nk; ++k) {
        temp = scale1 * kwave[k - 1];
        daughter[k - 1].re =
            norm * pow(temp, (double)m) * exp(-0.50 * pow(temp, 2.0));
        daughter[k - 1].im = 0.0;
      }
    } else if (re == 0) {
      for (k = 1; k <= nk; ++k) {
...
void psi0(int mother, double param, double *val, int *real) {
  ...
  if (mother == 0) {
    ...
    *val = sign * pow(2.0, (double)m) * factorial(m) /
           (sqrt(pi * factorial(2 * m)));
    ...
    *real = 1;
    ...
    if (m % 2 == 0) {
      ...
      *val = coeff * cwt_gamma(((double)m + 1.0) / 2.0) /
             sqrt(cwt_gamma(m + 0.50));
    } else {
      *val = 0;
    }
  }
...
double cdelta(int mother, double param, double psi0) {
  ...
  cwavelet(delta, N, dt, mother, param, s0, dj, jtot, N, wave, scale, period,
           coi);
...
void icwavelet(double *wave, int N, double *scale, int jtot, double dt,
               double dj, double cdelta, double psi0, double *oup) {
  int i, j, iter;
  double den, coeff;
